          ...
          >
            <div className="p-6 md:p-8">
              <div className="flex justify-between items-center mb-8">
                ...
                <button aria-label={t('settings.close') || "Close"} onClick={onClose} className="p-2 rounded-full bg-surface-container-highest hover:bg-surface-container-lowest text-on-surface-variant hover:text-primary transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-surface-container-high">
                  <X className="w-5 h-5" />
                </button>
              </div>
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between bg-surface-container-lowest/80 border border-outline-variant/20 rounded-xl p-1.5 backdrop-blur-sm">
                    <button
                      ...
                      className={`flex-1 py-2.5 rounded-lg font-label text-xs tracking-widest uppercase transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-surface-container-lowest ${language === 'es' ? 'bg-surface-container-high text-primary shadow-md scale-100' : 'text-on-surface-variant hover:text-on-surface scale-95 hover:scale-100'}`}
                    >
                      Español
                    </button>
                    <button
                      onClick={() => setLanguage('en')}
                      className={`flex-1 py-2.5 rounded-lg font-label text-xs tracking-widest uppercase transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-surface-container-lowest ${language === 'en' ? 'bg-surface-container-high text-primary shadow-md scale-100' : 'text-on-surface-variant hover:text-on-surface scale-95 hover:scale-100'}`}
                    >
                      English
                    </button>
                  </div>
