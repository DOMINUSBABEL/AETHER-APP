      ...
      <div className="w-full max-w-7xl relative min-h-screen flex flex-col md:flex-row bg-background shadow-2xl shadow-black/50 overflow-hidden">
        ...
        <header className="md:w-64 md:h-screen md:relative absolute top-0 w-full z-50 flex md:flex-col items-center justify-between md:justify-start px-6 md:px-0 py-4 md:py-8 h-16 md:h-auto bg-background/80 md:bg-surface-container-lowest/50 backdrop-blur-md border-b md:border-b-0 md:border-r border-outline-variant/10">
          ...
          <button 
            ...
            className="w-8 h-8 md:w-full md:h-auto md:px-8 md:py-4 md:rounded-none md:border-t md:border-l-0 md:border-r-0 md:border-b-0 md:bg-transparent rounded-full bg-surface-container-highest overflow-hidden border border-outline-variant/20 flex items-center justify-center md:justify-start md:gap-3 hover:bg-surface-container-high transition-colors focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none"
          >
...
function MobileNavItem({ icon, label, isActive, onClick }: { icon: ReactNode, label: string, isActive: boolean, onClick: () => void }) {
  return (
    <button 
      onClick={onClick}
      className={`flex flex-col items-center justify-center transition-all duration-300 group ${isActive ? 'text-primary' : 'text-secondary opacity-50 hover:opacity-100'} rounded-lg focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none`}
    >
...
function DesktopNavItem({ icon, label, isActive, onClick }: { icon: ReactNode, label: string, isActive: boolean, onClick: () => void }) {
  return (
    <button 
      onClick={onClick}
      className={`flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-300 group ${isActive ? 'bg-primary/10 text-primary' : 'text-secondary opacity-70 hover:opacity-100 hover:bg-surface-container-high'} focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none`}
    >
