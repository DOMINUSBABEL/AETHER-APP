...
## 2024-05-18 - [Accessibility on Visual Selectors]
**Learning:** Adding full keyboard accessibility to horizontally scrollable visual selectors requires combining `tabIndex={0}`, semantic `role="list"` and `role="listitem"`, and dynamic `aria-current="true"` state handling to make sure the structure and current selections are properly communicated.
**Action:** Consistently apply semantic roles to custom components masquerading as lists, and use `aria-current` rather than `aria-selected` for elements indicating a currently active item rather than a multi-selection mechanism.
