...
## 2024-04-28 - [Accessible Scrollable Selectors]
**Learning:** Found horizontally scrollable visual selectors (like the Zodiac signs list) that lacked keyboard focus and semantic meaning, making them difficult for screen reader and keyboard-only users to navigate and understand the active selection.
**Action:** Always make horizontally scrollable areas keyboard accessible by adding `tabIndex={0}` and standard `focus-visible` utility classes to the container. If the list acts as a selector but visually doesn't use standard controls, use semantic roles (`role="list"` on container, `role="listitem"` on items) and apply `aria-current="true"` to the active item.
