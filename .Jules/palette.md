...
## 2025-04-20 - [ARIA and Keyboard Navigation for Horizontally Scrollable Selectors]
**Learning:** Found horizontally scrollable visual selectors (like the Zodiac Selector in DailyScreen) that conveyed active state purely visually, lacking both keyboard navigability and semantic structure, making them completely inaccessible to keyboard and screen reader users.
**Action:** When implementing horizontally scrollable visual lists, always ensure the container has `tabIndex={0}` along with `focus-visible` outline styles so it can be scrolled via keyboard. Additionally, add `role="list"` and `aria-label` to the container, `role="listitem"` to its children, and indicate the active child with `aria-current="true"`.
