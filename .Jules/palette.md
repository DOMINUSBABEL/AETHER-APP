...
## 2024-05-18 - [Accessibility on Scrollable Visual Selectors]
**Learning:** Found horizontally scrollable visual selectors (like the Zodiac Selector) that were not keyboard focusable, making them inaccessible to keyboard users, and lacked semantic ARIA list roles, leaving screen reader users unaware of the content and the currently selected item.
**Action:** When implementing custom horizontal scrollable visual lists or selectors, always apply `tabIndex={0}` and `focus-visible` utility classes to the scrollable container. Additionally, provide semantic structure by adding `role="list"` to the container and `role="listitem"` to its children, using `aria-current="true"` to indicate the active state.
