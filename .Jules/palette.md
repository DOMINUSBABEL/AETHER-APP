...
## 2026-04-12 - Ambient Typography in Mystical Apps
**Learning:** Always verify if a UI element provides unique contextual information before hiding it from screen readers. Use attributes like `aria-current="true"` to indicate the active item in custom horizontal lists or selectors.
**Action:** Use `aria-current="true"` or `aria-selected="true"` for the active element in custom data displays that visually highlight the current context.
