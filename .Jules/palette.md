...
## 2024-05-09 - [Keyboard Access and Semantic Roles for Horizontal Lists]
**Learning:** Found horizontally scrollable visual selectors (like the Zodiac signs list) lacking keyboard focus and semantic structure, making them invisible and inaccessible to keyboard and screen reader users.
**Action:** Always add `tabIndex={0}` and `focus-visible` utility classes to horizontally scrollable containers to enable keyboard navigation. Additionally, apply `role="list"` to the container, `role="listitem"` to its children, and explicitly communicate the active item using `aria-current="true"` or `aria-selected="true"`.
