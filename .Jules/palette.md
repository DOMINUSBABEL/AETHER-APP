...
## 2024-10-24 - [Horizontal List Accessibility]
**Learning:** Horizontally scrolling areas for selection (like the zodiac sign selector) are often inaccessible to keyboard users because overflow containers don't naturally receive focus.
**Action:** Always add `tabIndex={0}` to horizontally scrollable element containers and give them `focus-visible` styling. For semantic clarity to screen readers, ensure these visual lists use explicit roles (`role="list"` and `role="listitem"`) and indicate the active option via `aria-current`.
