...
## 2024-06-25 - Horizontal Visual List Accessibility
**Learning:** Purely visual horizontal lists (like the zodiac signs selector) are common but frequently lack semantic roles and keyboard access, making them invisible to screen readers and difficult to navigate.
**Action:** When encountering visual lists, ensure they use `role="list"` and `role="listitem"`, receive keyboard focus via `tabIndex={0}` on the scrollable container, show focus with `focus-visible` classes, and mark the active element with `aria-current="true"`.
