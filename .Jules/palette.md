...
## 2025-04-25 - [Accessibility of Horizontally Scrollable Selectors]
**Learning:** Found custom visual lists (like the Zodiac Selector) that visually convey active state but lack focusability (`tabIndex`), semantic roles (`role="list"`, `role="listitem"`), and state communication (`aria-current`) for screen readers.
**Action:** When implementing custom visual selectors that users scroll through, always add `tabIndex={0}` and focus-visible utilities to the container, and use explicit semantic roles and `aria-current` to indicate the active item.
