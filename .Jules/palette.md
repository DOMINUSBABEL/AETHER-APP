...
## 2024-05-19 - [ARIA Attributes and Forms Labels]
**Learning:** Found instances in `LoginScreen` and `SettingsModal` where `htmlFor` and `id` were missing for some inputs like the time inputs, leaving labels disconnected.
**Action:** When creating forms, ensure all inputs have a corresponding `id` and labels are linked via `htmlFor`. Added `focus-visible` to time inputs to make keyboard navigation explicit.
