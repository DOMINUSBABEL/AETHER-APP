...
## 2025-02-28 - [Accessible Scrollable Selectors]
**Learning:** Horizontally scrollable visual selectors often lack semantic structure and are inaccessible to keyboard and screen reader users. The active state was only indicated visually, hiding context from screen readers.
**Action:** Always apply `tabIndex={0}` and explicit `focus-visible` states to horizontally scrollable interactive containers. Use `role="list"` on the container, `role="listitem"` on the elements, and `aria-current="true"` on the active item to semantically communicate its state.
