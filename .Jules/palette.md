...
## 2024-03-20 - Horizontal Scrollable Selectors Accessibility
**Learning:** Custom horizontally scrollable visual lists (like the Zodiac selector) need explicit semantic roles and keyboard accessibility since they are visually conveying list data.
**Action:** Always add `role="list"` and `tabIndex={0}` (with `focus-visible` utilities) to the scrollable container, `role="listitem"` to the children, and `aria-current="true"` to indicate the active item instead of hiding them.
