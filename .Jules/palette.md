...
## 2024-05-19 - Accessible Horizontally Scrollable Custom Lists
**Learning:** Custom visual selectors (like the Zodiac sign horizontally scrollable area) constructed using `div` elements are entirely opaque to screen readers and inaccessible via keyboard navigation if not properly annotated.
**Action:** When implementing custom horizontal scrollable lists, apply `tabIndex={0}` and standard focus-visible classes to the scrollable container. Also assign `role="list"` to the container, `role="listitem"` to its children, and explicitly indicate the active item with `aria-current="true"`. Use internationalization functions (e.g. `t()`) to supply an accessible `aria-label` to the container itself.
