...
## 2024-03-08 - [Accessibility] Semantic roles and focus states for visual scrollable selectors
**Learning:** Purely visual horizontal scrollable lists (like the zodiac signs selector in DailyScreen) fail both keyboard accessibility (they can't be scrolled without a mouse/touch) and screen reader accessibility (they have no semantic meaning and active state is conveyed only by color/opacity).
**Action:** Always add `tabIndex={0}` alongside Tailwind focus classes (`focus-visible:ring-2`, etc.) to the scrollable container. Additionally, explicitly declare `role="list"` on the container, `role="listitem"` on the children, and use `aria-current="true"` (or `aria-selected="true"`) to expose the visually active item to screen readers.
