...
## 2024-05-17 - [Accessible Horizontally Scrollable Visual Lists]
**Learning:** Found horizontally scrollable visual selectors (like the Zodiac signs) that relied solely on visual indicators for the active state and lacked keyboard focus ability on the container itself. This leaves screen reader users unaware of the list's purpose, its items, and the currently active item, while also making keyboard navigation difficult.
**Action:** When creating custom visual selectors or horizontally scrollable areas, always apply `tabIndex={0}` to the scrollable container along with focus-visible utility classes. Furthermore, use explicit semantic roles (`role="list"`, `role="listitem"`) and indicate the active item using `aria-current="true"` or `aria-selected="true"`. Finally, ensure the list container has an appropriate `aria-label`.
