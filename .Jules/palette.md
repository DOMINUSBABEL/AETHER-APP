...
## 2025-05-02 - [Keyboard Accessibility for Horizontally Scrolling Visual Selectors]
**Learning:** Discovered a custom horizontally scrolling selector (Zodiac signs) that conveyed its active state purely visually without keyboard access. A user tabbing through the page would bypass it entirely, and screen readers lacked list semantics to communicate its structure.
**Action:** When building visually scrollable selectors or lists, ensure keyboard accessibility by making the scrollable container focusable (`tabIndex={0}`) with proper focus styles (`focus-visible` utilities). Also, apply standard semantic roles (`role="list"` and `role="listitem"`) and indicate the active state to screen readers via `aria-current="true"`.
